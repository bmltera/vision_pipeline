...
try:
    ...
    raise RuntimeError("[ERROR] PyAV not found. Install with `pip install av`.\n" + str(e))

# ---------------------------- ONNX Runtime -----------------------------
try:
    import onnxruntime as ort
    if "CUDAExecutionProvider" not in ort.get_available_providers():
        print("[WARN] ONNX Runtime does not have GPU support. Install with: pip install onnxruntime-gpu")
except ImportError as e:
    raise RuntimeError("[ERROR] onnxruntime-gpu not found. Install with `pip install onnxruntime-gpu`.\n" + str(e))
...
class NVDecoder:
    ...
    def __iter__(self):
        for frame in self.container.decode(self.stream):
            start = time.time()
            arr = frame.to_rgb().to_ndarray()  # this does both decode + copy
...
def _lazy_load_models():
    ...
    if _YOLO_MODEL is None:
        _YOLO_MODEL = YOLO(str(YOLO_WEIGHTS))  # ONNX model loaded via ONNX Runtime
...
def process_video(video_input, yolo_conf: float, clip_conf: float, frame_interval: int):
    ...
    # Initialize decoder and writer
    decoder = NVDecoder(video_path)
    fps, width, height = decoder.fps, decoder.width, decoder.height

    fourcc = cv2.VideoWriter_fourcc(*"avc1")  # universally available
    ...
    # Return video path and metrics for Gradio
...
with demo:
    gr.Markdown("## Graffiti & Trash Detector 🖌️🗑️\nUploads a video and profiles GPU/CPU timings.")
    ...
    metrics_output = gr.Textbox(label="Performance metrics (s)", lines=10, interactive=False)
